...
// 1609
#define SSL_RFC8902_1609 0x01
// X.509
#define SSL_RFC8902_X509 0x02
...
// support is bitmap of SSL_RFC8902...
...
int SSL_get_1609_psid_received(SSL *s, uint64_t * const psid, 
                               size_t * const ssp_len, uint8_t ** const ssp,
                               unsigned char hashedid[8]);
int SSL_set_1609_sec_ent_addr(SSL *s, int port, const char* addr);

// Get IEEE 1609 certificate chain from a hash
typedef struct ssl_1609_cert_info_s {
   int           valid;        // 0 = record is invalid
   unsigned char hashedid[8];  // HASHEDID8_LEN
   char          id_name[64];
   char          valid_from[64];
   char          valid_to[64];
} ssl_1609_cert_info_t;

int SSL_get_1609_cert_chain(SSL *s, const unsigned char *cert_hashed_id, ssl_1609_cert_info_t *info, int info_size_in_bytes);
